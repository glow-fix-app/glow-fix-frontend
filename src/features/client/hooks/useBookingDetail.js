...
function buildBookingDetailView(booking, bookingId) {
  ...
  return {
    ...
    problemImages: booking?.problem_images ?? booking?.images ?? [],
    ...
    needsPayment: status === "accepted" && !isPaid,
    isCancellable: status === "confirmed" || status === "pending" || status === "accepted",
    ...
    lat: business.latitude ?? business.lat ?? null,
    lng: business.longitude ?? business.lng ?? null,
