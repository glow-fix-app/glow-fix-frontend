...
const COLORS = ['#3B82F6', '#F3F4F6', '#10B981', '#F59E0B', '#EF4444'];
...
export default function ProviderAnalyticsPage() {
  ...
  const { data: dashboardResp, isLoading: isLoadingDash } = useQuery({
    ...
    queryFn: () => providerApi.analyticsDashboard({ range: period }),
  });

  // Fetch Revenue
  const { data: revenueResp, isLoading: isLoadingRev } = useQuery({
    queryKey: ['analytics', 'revenue', period],
    queryFn: () => providerApi.analyticsRevenue({ range: period }),
  });

  // Fetch Bookings
  const { data: bookingsResp, isLoading: isLoadingBookings } = useQuery({
    queryKey: ['analytics', 'bookings', period],
    queryFn: () => providerApi.analyticsBookings({ range: period }),
  });

  // Fetch Top Services
  const { data: topServicesResp, isLoading: isLoadingTopServices } = useQuery({
    queryKey: ['analytics', 'top-services', period],
    queryFn: () => providerApi.analyticsTopServices({ range: period }),
  });

  const isLoading = isLoadingDash || isLoadingRev || isLoadingBookings || isLoadingTopServices;

  const stats = dashboardResp?.stats || {};
  const trends = dashboardResp?.trends || {};
  const revenueData = revenueResp?.daily || [];
  const topServices = (topServicesResp?.top_services || []).slice(0, 5);
    ...
    <div className="w-full pb-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          ...
          <Select.Popover className="bg-white border border-gray-200 rounded-xl shadow-xl p-1 z-[9999] min-w-[160px]">
            <ListBox>
              <ListBox.Item id="this_week" textValue="This Week" className="px-3 py-2 text-sm font-medium hover:bg-gray-50 rounded-lg cursor-pointer">This Week</ListBox.Item>
              <ListBox.Item id="this_month" textValue="This Month" className="px-3 py-2 text-sm font-medium hover:bg-gray-50 rounded-lg cursor-pointer">This Month</ListBox.Item>
              <ListBox.Item id="this_year" textValue="This Year" className="px-3 py-2 text-sm font-medium hover:bg-gray-50 rounded-lg cursor-pointer">This Year</ListBox.Item>
            </ListBox>
          </Select.Popover>
        </Select>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border border-gray-100 bg-white shadow-sm rounded-xl h-full flex flex-col">
          <div className="p-5 flex justify-between items-center border-b border-gray-50">
            <h3 className="text-sm font-semibold text-gray-900">Bookings by Type</h3>
          </div>
        ...
        <table className="w-full text-left border-collapse min-w-[600px]">
          <thead>
            <tr className="border-b border-gray-50 bg-gray-50/50">
              ...
              <th className="py-3 px-5 text-xs font-medium text-gray-500 text-right">Revenue</th>
            </tr>
          </thead>
             ...
             {(retentionData[0].new > 0 || retentionData[0].returning > 0) ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={retentionData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="new" name="New Customers" fill="#3B82F6" radius={[2, 2, 0, 0]} maxBarSize={60} />
                  <Bar dataKey="returning" name="Returning Customers" fill="#10B981" radius={[2, 2, 0, 0]} maxBarSize={60} />
                </BarChart>
              </ResponsiveContainer>
             ) : (
